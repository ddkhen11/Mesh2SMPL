...
Debug=True
normalWeighting=True
gamma = 1
alphas = np.linspace(200,1,20)

# ICP threshold for head scan = 10, whole body = 100
def nonrigidIcp(sourcemesh,targetmesh, threshold=100.0):
    ...
    for num_,alpha_stiffness in enumerate(alphas):
        ...
        # dyn_threshold = max( dyn_threshold/(num_ + 1), threshold/2)
        dyn_threshold -= threshold / (L + 1)
        print(dyn_threshold)
        for i in range(3):
            ...
            mismatches = np.where(distances>dyn_threshold)[0]
            ...
            if normalWeighting:
                ...
                # print(crossNormalsNorm.shape)
                badNorms = np.where(crossNormalsNorm <= 0)[0]
                crossNormalsNorm[badNorms] = 0.000000001
